...
| `scripts/build_won_sheets.py` 의 `CHOICES` | `src/common/won.py` 의 같은 목록(`CLIENT_ID_BANDS`, `INDUSTRIES` …), 그리고 `--refresh-derived`. 「갱신 계획」은 예외로 여기 CHOICES 에만 있다 — 콘솔에서 없어진 칸이라(0073) 시트 드롭다운이 유일한 사용처다 | 드롭다운에 없는 값을 콘솔이 쓴다. `tests/test_won_sheets.py:124` 가 고객 종류만 잡아 준다 |
