...
- **Client ID와 문의 행 키는 다르다.** 같은 회사의 모든 문의는 Client ID를 공유하고,
  `Inquiry ID`가 append 재시도와 단계 갱신의 고유키다(0085). 새 Client ID 발급은 PostgreSQL
  advisory transaction lock으로 겹치지 않게 하고, Sheets의 최대값 조회와 append를 보호하는
  `_APPEND_LOCK`은 프로세스 안에서만 유효하므로 **Sheets를 쓰는 워커는 하나로 유지한다.**
...
| `scripts/build_won_sheets.py` 의 `CHOICES` | `src/common/won.py` 의 같은 목록(`CLIENT_ID_BANDS`, `PAYMENT_METHODS` …), 그리고 `--refresh-derived`. 「갱신 계획」과 「산업 분야」는 예외로 여기 CHOICES 에만 있다 — 콘솔에서 없어진 칸이라(0073 · 0124) 시트 드롭다운이 유일한 사용처다 | 드롭다운에 없는 값을 콘솔이 쓴다. `tests/test_won_sheets.py:124` 가 고객 종류만 잡아 준다 |
...
| Inbound DB 문의 행을 만드는 곳 | **두 곳이다**: `src/agents/inbound.py` (실시간)과 `src/agents/sheet_sync.py` (백필). 둘 다 `client_id`와 `inquiry_key`를 써야 한다 | 같은 회사 재문의가 기존 행을 덮거나 단계 갱신이 다른 문의를 건드린다 |
