...
| [04 수주고객과 돈](04-수주고객과-돈.md) | 계약·크레딧·분납·클레임. **돈에 관한 값은 거의 전부 저장하지 않고 매번 다시 계산한다** 는 원칙과 그 예외(그날 환율), 그리고 금액이 조용히 사라지는 자리. |
...
2. ~~**수주 고객 상세에서 「갱신 계획 · 비고」 한 줄을 저장하면 옛 원화 계약의 금액이 통째로
   ...
   **부분 폼을 받는 라우트에서 폼에 없는 칸을 만지면 안 됩니다.** 원래 증상: 부분 폼이 금액 칸을 안 보내는데 `_one_amount_per_currency` 가 마지막에 무조건
