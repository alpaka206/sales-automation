...
TABS: list[dict] = [
    ...
    },
    {
        ...
        "array": {
            ...
            # 산정 크레딧 = **계약 금액(L)** ÷ 분당 단가 × 60. 통화가 다르면 계약 행에 박아
            # 둔 환율로 단가를 계약 통화로 옮긴다. 이 탭은 계약 탭의 행을 그대로 따라가므로
            # 조회 없이 같은 행을 위치로 가리키면 된다 — 복합키 VLOOKUP 을 다섯 번 할 이유가
            # 없다. 금액이나 단가가 없으면 계산하지 않는다. 0 으로 두면 "산정 0 ≠ 계약
            # 크레딧" 이 되어, 정보가 없을 뿐인 행이 전부 틀린 것처럼 붉게 뜬다.
            #
            # **M(공급가)에서 L(계약 금액)으로 옮겼습니다** (2026-09-21). 계약 금액이 VAT
            # 포함 한 칸이 되면서 콘솔의 분당 단가가 그 금액에서 나오는데(`won.unit_price`),
            # 여기만 공급가를 계속 보면 M = L ÷ 1.1 이라 산정 크레딧이 **모든 행에서** 계약
            # 크레딧 ÷ 1.1 로 나옵니다. 확인 열이 전부 「크레딧 산정 불일치」로 찍히면, 진짜
            # 어긋난 행을 눈에 걸리게 하려고 만든 이 열이 죽습니다.
            "E": _array(
                f"IF(ISNUMBER($A$2:$A)*ISNUMBER('{CONTRACTS_TAB}'!$L$2:$L)"
                f"*ISNUMBER('{CONTRACTS_TAB}'!$O$2:$O)*('{CONTRACTS_TAB}'!$O$2:$O>0),"
                f"IFERROR(ROUND('{CONTRACTS_TAB}'!$L$2:$L/"
