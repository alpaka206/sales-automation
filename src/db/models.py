...
class PolicySource(Base):
    ...
    # 있는 행입니다.
    # ``subject`` 가 여기 있었습니다 (이관 0118). 이 문서를 근거로 회신할 때의 **고정 메일
    # 제목**이었고, 두 번 사고를 냈습니다: 제목을 든 문서가 둘이면 가나다순으로 앞선 쪽이
    # 이겨서 참고 문서가 실제 회신 서식을 제쳤고(2026-08-26), 문서 제목은 운영자가 쓴 고정
    # 문장이라 한국어 문의에 영어 제목이 나갔습니다(msg 62). 지금 제목은
    # ``common.subjects.reply_subject`` 하나가 정합니다 — 「RE: <고객이 쓴 제목>」.
    #
    # **언제 이 문서를 쓰는가.** 문서를 고르는 것은 모델이고, 모델이 볼 수 있는 것은 본문이
    # 아니라 인덱스 한 줄(slug·title·summary)입니다. 이 칸이 그 요약이고, 비우면 예전처럼
    # 본문 앞부분이 잘려 들어갑니다.
    #
    # **사람이 적는 칸이 아닙니다** (2026-09-10 운영자 지시). 저장할 때
    # ``knowledge.usage_note_from_body`` 가 본문을 읽어 「쓰는 경우: … / 담긴 것: …」 한
    # 줄을 만듭니다 — 사람이 적게 두면 안 적힌 행이 반드시 생기고, 바로 표로 시작하는
    # 문서에서는 본문 앞 400자가 아무것도 안 말해서 라우터가 그 문서를 못 고릅니다.
