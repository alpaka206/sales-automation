...
- **MQL / PQL 은 구독 플랜이 정하고, 저장하지 않는다** (2026-09-02 운영자 지시).
  ...
  - **워크북으로 나가는 길은 건드리지 않았다.** 시트의 Pipeline 열은 **수식**이라 스스로
    같은 판단을 하고(`_pipeline_formula`), `update_inbound_stage(pipeline=...)` 이 값을 써
    넣으면 그 수식이 지워진다. 그 인자는 지금도 저장된 열을 그대로 읽는다 — 파생값을
    흘려보내면 그날부터 시트의 수식이 한 줄씩 정적인 값으로 굳는다. **그 경로에는 이미
    문제가 하나 있다**(이번 변경과 무관하게 예전부터): 워크북 전체 임포트가 시트의 계산
    결과를 그 열에 되받아 적으므로(`sheet_sync`), 그 뒤의 단계 동기화가 그 값을 시트로
    돌려보내 수식을 덮는다. 고치려면 그 인자를 없애 시트의 수식이 늘 이기게 하면 되는데,
    그 열은 영업팀의 것이고 수식에는 콘솔에 없는 `재계약` 가지가 있다 — 지우기 전에 물어라.
