...
`_draft_reply` 안의 CODE GUARD 세 개(`inbound.py:881-917`)이고, 순서가 사양입니다.
`ensure_korean` → `apply_editable_tokens` → 금액 제거 → 제목. 토큰 치환이 번역 **뒤**인 이유는
...
### `INBOUND_DOMAIN_REANALYZE_DAYS` 는 아무도 읽지 않는다

`src/common/config.py:225` 에 있지만, 실제 만료는 `domain_enrichment.py:19` 의 모듈 상수
`_MAX_AGE_DAYS = 90` 입니다. `.env` 에서 30으로 바꿔도 90일 그대로입니다.

### Client ID 발급이 `agents/client_ids.py` 를 지나지 않는다 — 확인 필요

CLAUDE.md 의 불변식은 "Client ID 는 고객사 하나에 하나" 이고, 그 판정을 `agents/client_ids.py`
가 합니다(그 연락처의 번호 → 같은 회사 도메인의 형제 → 새 발급). 그런데 **인바운드 접수
경로는 그 파일을 부르지 않습니다.**

- `inbound.py:355` → `sheet_sync.reserve_inbound_client_id(conv_id)`
- 그 함수(`src/agents/sheet_sync.py:483-517`)는 시트의 다음 번호(`suggest_inbound_client_id`)와
  로컬 `Conversation.sheet_client_id` 최댓값+1 중 큰 쪽을 잡습니다. **회사·도메인·연락처를
  전혀 보지 않습니다.**
- `client_id_for_inquiry` / `find_existing_client_id` 를 부르는 곳은 프로덕션 코드에 없습니다
  (`won_customers.py:120` 은 `next_client_id` 만 씁니다).

결과: 같은 회사가 두 번 문의하면 대화 두 개에 Client ID 두 개가 붙습니다(연락처의
`Contact.sheet_client_id` 만 첫 번호를 유지합니다 — `inbound.py:416-417` 이 비어 있을 때만
채우므로).

의도와 코드 중 어느 쪽이 최신인지는 코드만으로 판단할 수 없어 **확인 필요**로 남깁니다.
고치기 전에 물어야 할 것: 이미 나간 번호를 합칠 것인가, 아니면 앞으로만 재사용할 것인가.
