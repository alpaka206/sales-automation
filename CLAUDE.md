...
- **개인함 메일은 붙을 자리가 있으면 수집하면서 바로 붙는다** (2026-09-09 운영자 지시:
  ...
  **허브스팟 티켓 번호가 있는지도 안 본다.**
  ...
  - **같은 메일의 사본은 하나만 남는다 — 자는 `ticket_history.same_mail` 하나** (2026-09-15,
    ...
    제목(Re:/Fwd: 뗀 것) · 10분 안이면 같은 메일이다. 지메일 `Date`(보낸 쪽 시계)와 허브스팟
    ...
    개인함으로 보낸 회신의 사본은 지메일 id(`messages.smtp_message_id`)로 거른다. 다른 사서함의
    `gmail:` 줄끼리는 같은 초를 요구한다(같은 메일은 `Date` 가 글자까지 같다). 그리고 **우리가
