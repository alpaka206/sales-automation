...
def _contract_row(contract: ClientContract) -> _Row:
    return _Row(
        ...
        entered={
            ...
            # M(공급가)은 없어진 값입니다 (2026-09-22 운영자 지시, 이관 0127). **열은 남기고
            # 비웁니다** — 열을 지우면 N~AM 이 한 칸씩 밀려 들어가고 예외는 안 나며, owned
            # 에서 M 을 빼면 지워진 계약의 옛 공급가가 `plan_tab` 의 재사용 행에 그대로
            # 얹혀 그 행의 총액과 안 맞습니다(I·N·P 와 같은 이유). 「검증」 탭의 산정
            # 크레딧은 이미 L ÷ O × 60 이라 이 열을 안 봅니다.
            "M": "",
