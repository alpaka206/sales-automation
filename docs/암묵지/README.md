...
| [02 검토와 발송](02-검토와-발송.md) | 승인 → 클레임 → HubSpot Conversations 스레드 선택 → 발송 후 처리. 중복 방지를 위한 `delivery_unknown`과 번역·수신자 가드. |
...
- **`src/agents/report.py` 를 부르는 스케줄러가 없다.** 유일한 호출자는 수동 report API이고,
  `render.yaml` 에 cron이 없다. 보고서는 `data/reports`에 저장만 하며 별도 이메일을 보내지 않는다.
...
> 2026-08-24 변경: 문의 접수 직후 자동 메일은 더 이상 발송하지 않습니다. 아래 `auto_ack` 도식은 제거 전 설계와 과거 메시지 형식을 설명합니다.
