...
#
# **닫는 괄호는 주소가 아닙니다.** 링크는 `[Calendly](https://…)` 로 나가는데, 여기서
# `)` 까지 주소로 집으면 모델이 보는 것은 짝이 안 맞는 `[Calendly](%%0%%` 이 되고, 모델은
# 그 괄호를 닫아 줍니다 — 되돌리면 `[Calendly](https://…))` 입니다. 실제로 그렇게 나왔고
# (2026-08-26), 발송 직전 `canonicalize_contact_links` 가 그 줄을 통째로 다시 만들어 주는
# 덕에 고객에게는 안 갔을 뿐 검토 화면에는 그대로 보였습니다. 대괄호도 같은 이유로 뺍니다.
# `prompts._LINK_URL_RE` 와 같은 글자 집합입니다.
