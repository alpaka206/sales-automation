...
> (`stage_sync._retire_superseded_drafts`) 셋뿐이다. 발송 경로의 마지막 경계는
> `EMAIL_SENDING_ENABLED`와 `LIVE_EXTERNAL_WRITES`이며, 둘 다 켜져 있어야 실제 수신자에게
> 전달된다. 특정 내부 주소로 수신자를 바꾸는 테스트 모드는 없다.

전제로 읽을 것: CLAUDE.md 의 「External-write safety」 · 「Human-approved email delivery」 ·
...
flowchart TD
    ...
    H --> I["senders.send()"]
    I --> K["enforce_send_language<br/>검토된 목표 언어인지 확인"]
    K --> L["enforce_first_reply_no_price<br/>첫 회신이면 금액 줄 삭제"]
    L --> M["send_smtp()"]
    M --> N{"email_delivery_enabled()"}
    N -->|False| O["SMTPSendingDisabled<br/>소켓조차 열지 않음"]
    N -->|True| P["SMTP DATA"]
...
### 메일 안전장치는 주소를 바꾸지 않고 전송을 차단한다

`send_smtp()`는 `email_delivery_enabled()`를 가장 먼저 검사한다. 코드의 긴급 스위치와
`LIVE_EXTERNAL_WRITES` 중 하나라도 꺼져 있으면 `SMTPSendingDisabled`를 내고 SMTP 소켓을
열지 않는다. 이후 SMTP 자격증명도 별도로 검사한다. 테스트 주소 치환은 없다.

**어기면**: 이 검사를 `_build_message` 뒤로 옮기면, 서명·제목 인코딩이 잘못된 메일에서는
...
`test_mode = not delivered`이다. 안전 스위치 때문에 아예 안 보냈으면 `test_sent`다.
...
### 발송 후 처리는 `test_mode`와 무관하다

메일이 안전 스위치로 억제돼도 로컬 상태는 남는다. HubSpot·워크북 같은 외부 목적지는 각자의
`guard_external_write`가 막는다. 여기서 다시 `not test_mode`로 묶지 않는다.
...
> 2026-08-24 변경: 승인된 초안의 실발송과 실제 수신자 전달을 켰습니다. 외국어 초안은 `번역하기` 완료 전 승인할 수 없고, 발송 경로의 숨은 자동 번역도 제거했습니다. 아래 pre-launch·강제 테스트 수신자 설명은 이전 운영 상태 기록입니다.
