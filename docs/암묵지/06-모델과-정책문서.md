...
**어기면 / 틀리면**: 오검출은 그 스레드에서 영구적입니다. 베트남어 문의가 `en` 으로 잡히면
접수확인이 영어로 나가고, 운영자가 검토 화면에서 베트남어로 직접 고쳐 써도
`enforce_send_language` 가 "본문 언어(ko) ≠ target(en)" 으로 보고 그 베트남어를 **영어로
번역해서** 보냅니다(`src/integrations/senders/__init__.py:43-54`). 화면에는 아무 경고도
없고, 로그에 `Send guard: translated message ... to target 'en'` 한 줄만 남습니다.
라틴 문자 문의만 LLM 판정을 거치므로(`src/llm/language.py:47-71`) 위험 구간은 베트남어 ·
스페인어 · 인도네시아어처럼 라틴 표기 언어입니다.

### 초안 후처리 네 단계는 이 순서여야 한다

`ensure_korean` → `apply_editable_tokens` → `strip_price_sentences` → 제목 결정
(`src/agents/inbound.py:881-917`).

- **번역이 토큰 치환보다 먼저**여야 합니다. 코드에 그렇게 적혀 있습니다("translation would
...
- 제목은 마지막입니다. 모델이 준 `draft.subject` 는 무조건 버려집니다.
...
### 정책 문서 편집에는 되돌리기가 없다 — 이메일 템플릿에는 있다

`email_templates` 는 수정·삭제 전에 `EmailTemplateRevision` 을 남깁니다
(`src/api/routes/email_templates.py:46-61`). `policy_sources` 에는 개정 테이블이 없고,
`knowledge_document_revisions` 테이블은 존재하지만 **아무도 쓰지 않습니다** —
`_upsert_knowledge` 는 스냅샷을 남기지 않고 덮어씁니다(`src/agents/policy_sync.py:96-109`).
모델 docstring(`src/db/models.py:303-309`)이 "매 수정 전에 한 행을 쓴다" 고 말하는데 사실이
아닙니다. **정책 문서 본문을 붙여넣기로 갈아치우기 전에 어딘가에 복사해 두세요.**
...
### 번역 출력은 2,000 토큰에서 잘리고, 잘린 결과가 그대로 본문이 된다

초안은 `max_tokens=4000` (`src/agents/inbound.py:878`), 번역은 `max_tokens=2000`
(`src/llm/translate.py:76`, `:107`). Gemini 가 한도에서 끊어도 텍스트는 비어 있지 않으므로
`translated` 가 truthy 가 되고, `enforce_send_language` 가 그것으로 본문을 교체합니다
(`src/integrations/senders/__init__.py:51-54`). 한국어는 토큰이 조밀하고 영어는 늘어나므로
긴 초안일수록 위험합니다. **긴 회신을 보낼 일이 잦아지면 이 두 숫자를 같이 올리세요.**
...
### 모델 호출 하나가 DB 왕복 두 번을 덧붙인다

`_dispatch_once` 가 매번 `log_usage()` (`llm_usage` INSERT + commit,
`src/llm/pricing.py:59-81`) 와 `_log_event()` (`events` INSERT + commit,
`src/llm/client.py:250-265`) 를 부릅니다. 각각 자기 세션을 열고 커밋합니다. 인바운드 한
건이 모델을 열 번 부르면 여기서만 **스무 번의 왕복**이 붙습니다. 둘 다 `try/except` 로
감싸 있어 DB 가 느리면 실패가 아니라 **지연**으로 나타납니다.

### 발송 버튼이 이벤트 루프에서 Gemini 를 기다린다

`senders.send()` 는 async 인데 `enforce_send_language` 를 그냥 부릅니다
(`src/integrations/senders/__init__.py:200`). 그 안에서 `translate_to` → `LLMClient` →
동기 HTTP, 그리고 일시적 오류면 `time.sleep(2)` (`src/llm/client.py:218`). 같은 함수의
SMTP 는 `asyncio.to_thread` 로 감싸 두었습니다(`:204`) — 번역만 안 감싼 것입니다.
`SEND_WORKER_ENABLED=false` 면 이 경로가 웹 요청 안에서 돕니다
(`src/api/routes/messages.py:728-735`).
