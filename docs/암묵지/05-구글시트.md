...
> 지급 현황 · 결제 현황 · 클레임 · 히스토리」 다섯 탭 전체가 DB 와 맞춰진다. 반대 방향(시트 →
...
flowchart TB
  ...
  subgraph 시트에서 콘솔로
    H[sheet_to_db.import_from_sheet<br/>한 번만] --> I[clients / client_contracts / 회차 / 클레임]
...
계약·회차·클레임 탭으로 옮겼다). 그런데 `append_order_row` 는 여전히
