...
- **회신이 어느 주소에서 나갈지 운영자가 고른다** (2026-09-02 지시, 이관 0105).
  ...
  - **같은 인박스의 주소만 고를 수 있다**(`_account_for_thread`). 폼 폴백이 이미 지키던
    규칙과 같은 규칙이다: 인박스가 다르면 허브스팟이 받아 줄지 알 수 없고, 받아 준다면 그건
    그것대로 남의 인박스에 우리 메일이 서는 것이라 그 팀 화면에 안 보인다.
  ...
  - **목록은 서버가 만든다**(`list_reply_senders`, `GET /api/ui/messages/{id}/senders`).
    읽기만 하므로 그 라우트로는 메일이 안 나간다. 화면이 스스로 목록을 지으면 고를 수는
    있는데 발송이 거절하는 값이 생긴다. 못 가져오면 고르개가 안 뜰 뿐 발송은 예전대로다.
  ...
  - **실측 (2026-09-02, B2B 파이프라인)**: 티켓 100건이 **전부** `Inbox` 인박스 스레드를
    갖는다(GTM Marketing 만 있는 티켓은 0건). 그래서 `perso.ai@estsoft.com` 을 `Inbox`
    인박스로 옮기면 **100%** 그 주소로 나간다. 지금처럼 GTM Marketing 에 있으면 32%다
    (50건 중 16건. 14건은 GTM 스레드는 있으나 메일이 오간 적 없고, 20건은 GTM 스레드가 없다).
