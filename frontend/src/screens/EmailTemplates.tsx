...
function Editor({ id, data, onDone }: {
  ...
  return (
    <>
      ...
      {/* 「7일 동안은 목록에서 되돌릴 수 있지만」이 아래 문장에 있었습니다 — 이관 0100 이
          하드 삭제로 바꾸면서 거짓이 됐습니다(휴지통도, 되돌리기 버튼도 없습니다). 사라진
          본문은 판본 이력에 남고, 그 이름은 그 자리에서 다시 쓸 수 있습니다. */}
      {confirming && data && (
        <DeleteDialog
          name={data.name}
          ...
          warning={codeResolved
            ? `발송 경로가 이 행을 「${data.key}」 라는 이름으로 찾습니다. 지워도 그 조회는 ` +
              ...
              "회신이 {{MEETING_LINK}} 같은 토큰으로 끝납니다. 쓰지 않을 생각이라면 내용을 " +
              "비우는 쪽이 되돌리기 쉽습니다."
            : undefined}
          note="누르면 이 행이 그 자리에서 사라집니다 — 7일 휴지통은 없습니다. 본문은 판본 이력에 남고, 같은 키로 다시 만들 수 있습니다."
        />
      )}
    </>
  );
