...
- **회신이 어느 주소에서 나갈지 운영자가 고른다** (2026-09-02 지시, 이관 0105).
  ...
  - **기본 발신 주소는 다른 인박스에도 한 번 두드려 본다** (2026-09-03 운영자 요청,
    ...
    - **화면이 그 시도를 적는다**(`fallback_address`) — 「자동 — perso.ai@estsoft.com
      (거절되면 support@perso.ai)」. 발송과 고르개가 `cross_inbox_attempt` 한 함수를 같이
      쓴다. 안 그러면 화면과 실제가 갈린다(2026-09-03 에 그걸로 한 번 당했다).
