...
### `frontend/test/quote.golden.json` 은 갱신하는 파일이 아니다

`frontend/test/quote.test.ts:5-18`. 1,512 행은 **React 포팅 전** 계산기 스크립트가 스텁 DOM 위에서 실제로 그려낸 값입니다. 실패는 "오늘 코드가 영업팀이 쓰던 계산기와 다른 가격을 부른다" 는 뜻이고, 그건 누군가 일부러 내려야 할 결정입니다.

여기에 **양방향 잠금**이 걸려 있습니다: golden 파일은 자기가 쓰인 가격표(`policy`)를 같이 들고 있고, `tests/test_quote_tiers.py:83-101` 이 그 사본과 `src/common/quote_tiers.py` 를 대조합니다. 파이썬에서 티어 가격을 고치면 **파이썬 테스트**가 먼저 터집니다 — golden 이 옛 가격표를 설명한 채 조용히 통과하는 일을 막는 장치입니다.

포맷도 검사 대상입니다(`quote.test.ts:41-42`): 고객에게 `"1,200"` 과 `"1200.4"` 는 다른 답입니다.
...
### `/api/ui/won-customers` 의 `selectinload` 세 줄을 지우지 마라

`ui_api.py:811-821` 에 실측이 적혀 있습니다. 계약만 미리 읽고 그 **아래**(크레딧 회차·결제 회차·클레임)를 안 읽었더니, 고객 40 · 계약 80 짜리 장부 하나를 그리는 데 쿼리가 **163 번** 나갔습니다. 왕복 200ms 면 30초입니다. 세 줄을 더해 163 → 6 이 됐고 결과는 한 글자도 다르지 않습니다.

**함정**: `_won_client` 이 부르는 `won.open_claims` 는 계약마다 클레임을 훑고, 활성 계약 하나는 회차 세 종류를 전부 만집니다. 여기에 파생값을 하나 더 붙이면 그 자리에서 N+1 이 되살아납니다 — 화면에는 "수주 고객 목록이 느리다" 로만 보입니다.
...
수주 고객도 같은 판단입니다(`ui_api.py:659-667`): 상세 8개 섹션이 각각 요청이면 열 때 왕복이 여덟 번입니다. 파생값(다음 지급일·수금율·월간 매출·고객 종류)은 **저장하지 않고 응답에서 계산**합니다 — 저장하면 원본이 바뀌었는데 파생값은 안 바뀐 행이 생기고, 그건 화면에 안 보입니다.
...
`Overview.tsx:7-11` 도 같은 원칙의 다른 얼굴입니다: 전체 대시보드는 **자기 데이터가 없습니다**. 모든 숫자는 안쪽 화면의 builder 에서 오고 그 화면으로 링크됩니다. 요약이 요약 대상과 어긋나면 없느니만 못합니다.

### 사이드바 활성 판정이 규칙 한 줄인 이유

`Shell.tsx:70-75`. 정확 일치만 보면 `/won-customers/2102` 상세에서 사이드바가 아무 데도 강조하지 않아 내가 어느 화면인지 사라지고, 접두사만 보면 `/` 가 모든 경로의 앞부분이라 문의 대시보드가 항상 켜집니다. 규칙은 "정확히 같거나 `path + '/'` 로 시작" 하나 — 루트는 `"//"` 가 되어 어디에도 안 걸리고, `/won-customers` 는 `/customers/` 로 시작하지 않아 형제도 안전합니다. `협상중 고객`과 `리드 히스토리`는 경로가 같고 쿼리만 다르므로 `location.search` 도 같이 봅니다.
...
| `src/common/quote_tiers.py` (가격) | `frontend/test/quote.golden.json`, `tests/test_quote_tiers.py:83` | golden 이 옛 가격표를 설명하지 않도록 |
| `frontend/src/lib/quote.ts` | `frontend/test/quote.test.ts` (재생성 금지) | 견적 숫자가 바뀌면 실패해야 합니다 |
