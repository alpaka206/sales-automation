...
**어기면**: 서른 칸짜리 계약 폼을 다 채우고 Escape 를 스치듯 눌러 전부 날립니다. 소통 히스토리 폼은 값을 DOM 에만 들고 있어 더합니다.
...
- **차트 라이브러리 없음**: 고객 인사이트의 사각형 일곱 개(`Operations.tsx:88-90`)도, 수주
  고객의 월별 영역 차트(`won/MonthlyArea.tsx` — 2026-09-02 에 막대에서 면으로 바뀌었습니다.
  MRR 은 추세로 읽는 값이고, 무엇보다 면은 New 를 위에 **쌓을 수 있습니다**)도 직접
  그립니다. 운영자가 라이브러리를 써도 된다고
  한 뒤에도 안 쓴 이유는 값입니다: Recharts·Chart.js 는 gzip 90~110KB 이고 번들이 127KB
  입니다. 차트 하나에 두 배로 만드는데, 필요한 것은 면 둘·0선·눈금·툴팁이 전부라
  라이브러리가 하는 일의 대부분을 안 씁니다. 색·글꼴·모서리도 `won.css` 토큰에서 오는데
  라이브러리는 자기 기본값을 들고 옵니다 — 맞추는 작업이 그리는 작업보다 큽니다.
  애니메이션·확대·여러 차트 종류가 필요해지면 그때가 들일 때입니다.
...
「소통 히스토리를 남길 수 있는 단계」가 대표입니다. 보드의 `+` 버튼(`Board.tsx:103`)과 티켓 상세의 `추가하기`(`MessageDetail.tsx:115`)가 같은 `manual_log_stages` 를 씁니다 — 서버의 `MANUAL_LOG_STAGES` 한 곳에서 오고(`ui_api.py:188-190`), 화면마다 "New 는 빼고" 를 따로 적으면 언젠가 어긋납니다.
