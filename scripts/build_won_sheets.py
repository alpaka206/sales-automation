...
CHOICES = [
    ["고객 종류", "산업 분야", "담당부서", "플랜 상태", "매출 인식", "계약서 유형", "통화", "결제 수단", "결제 방식", "플랜", "갱신 계획", "크레딧 상태", "결제 상태", "클레임 진행상황"],
    ...
    ["Interactive", "MCN", "AX", "사용 중단", "", "결제 시 약관 및 협의 내용 동의", "", "직접거래", "", "Business Tier 3", "미정", "", "", "조치 완료"],
...
TABS: list[dict] = [
    ...
    },
    {
        ...
        "text": ["D", "Z", "AL"],
        "dv": {"E": "매출 인식", "K": "통화", "N": "통화", "Q": "결제 수단", "R": "결제 방식",
               "W": "갱신 계획", "AB": "플랜"},
        ...
        "note": {
            ...
            "J": "1분 = 60크레딧 고정. 계약서에 적힌 크레딧을 그대로 적습니다. "
                 "검증 탭이 계약 금액 ÷ 분당 단가 × 60 으로 다시 계산해 맞는지 봅니다.",
    ...
    },
    {
        ...
        "array": {
            ...
            # 산정 크레딧 = **계약 금액(L) ÷ 분당 단가(O) × 60.** 이 탭은 계약 탭의 행을
            # 그대로 따라가므로 조회 없이 같은 행을 위치로 가리킨다 — 복합키 VLOOKUP 을 다섯
            # 번 할 이유가 없다. 금액이나 단가가 없으면 계산하지 않는다: 0 으로 두면 "산정 0 ≠
            # 계약 크레딧" 이 되어, 정보가 없을 뿐인 행이 전부 틀린 것처럼 붉게 뜬다.
            #
            # **M(공급가)에서 L(계약 금액)으로 옮겼다** (2026-09-21). 계약 금액이 VAT 포함 한
            # 칸이 되면서 콘솔의 분당 단가가 그 금액에서 나오는데(`won.unit_price`), 여기만
            # 공급가를 계속 보면 M = L ÷ 1.1 이라 산정 크레딧이 **모든 행에서** 계약 크레딧 ÷
            # 1.1 로 나온다. 확인 열이 전부 「불일치」로 찍히면, 진짜 어긋난 행을 눈에 걸리게
            # 하려고 만든 이 열이 죽는다.
            #
            # **통화 환산 가지도 같이 걷어냈다** — 그게 이 열을 실제로 죽이고 있었다.
            # `IF(N=K, O, IF(N="USD", O*P, O/P))` 로 분당 단가 통화(N)와 적용 환율(P)을
            # 봤는데, 그 두 칸은 없어진 칸이라 `won_sheets` 가 **빈칸으로 내보낸다**
            # (`"N": ""` · `"P": ""`). 그래서 언제나 `N ≠ K` → `O ÷ 빈칸` → 오류 →
            # `IFERROR` → `""` 였고, **산정 크레딧 열이 전 행 비어 있었다**(2026-09-21 실측).
            # 단가는 언제나 계약 통화다(`won.unit_price` 는 계약 금액에서 나온다) — 나눌 것은
            # O 하나다.
            "E": _array(
                f"IF(ISNUMBER($A$2:$A)*ISNUMBER('{CONTRACTS_TAB}'!$L$2:$L)"
                f"*ISNUMBER('{CONTRACTS_TAB}'!$O$2:$O)*('{CONTRACTS_TAB}'!$O$2:$O>0),"
                f"IFERROR(ROUND('{CONTRACTS_TAB}'!$L$2:$L/'{CONTRACTS_TAB}'!$O$2:$O*60,0),\"\"),\"\")"
            ),
        ...
        "note": {
            ...
            "E": "계약 금액 ÷ 분당 단가 × 60 을 시트가 다시 계산한 값입니다. 계약 크레딧과 "
                 "다르면 둘 중 하나가 틀린 것입니다.",
