...
- **수주 고객은 Client ID 로 묶인다.** `clients` 아래 `client_contracts`(차수별), 그 아래 크레딧 지급 회차·분납 회차. 소통 히스토리만 예외로 고객 단위라 새 테이블을 만들지 않고 `customer_interactions` 에 `contract_seq` 만 붙였다 — 협상 단계 대화가 계약보다 먼저 쌓이고 그대로 이어져야 한다(0065).
  ...
  - **계약 금액은 한 칸이고 그 값이 VAT 포함 총액이다** (2026-09-21 운영자 지시: 「계약금액은 모두 VAT 포함만 으로 바꿀거야」). `won.total_amount` 이 그 칸을 그대로 돌려주고, 공급가는 저장하지 않고 **총액 ÷ 1.1** 로 되짚는다(`won.supply_amount`) — 워크북의 그 열은 회계가 합계를 내는 칸이라 비면 그 행만 조용히 빠지고, 화면은 같은 값 옆에 「총액에서 역산」이라고 적는다. `vat_applicable` 은 **남는다**: 「공급가라는 것이 있는 계약인가」를 아는 곳이 그 칸뿐이라, 같이 지우면 해외 계약의 공급가 칸이 0 으로 채워져 시트와 CSV 로 나간다. 분당 단가도 이 금액에서 나온다(`won.unit_price`).
    - **그전에는 금액이 둘이었다** — `amount_excl_vat` 와 「어느 쪽이 계약서에 적힌 금액인가」를 든 `vat_included`. 국내 계약서가 공급가로도 총액으로도 적혀서, 모르면 총액이 10% 부풀거나 단가가 10% 낮아지고 **화면 어디에도 그게 안 보였기** 때문이다. 이관 0123 이 둘을 지우면서 **미포함 기준이던 계약의 금액을 그 숫자 그대로 계약금액으로 옮겼고**(운영자가 고른 쪽이다 — 그 건들의 총액·월 MRR·예상 MRR 이 10% 내려앉는다는 표를 보고 골랐다), 사라지는 사실은 **계약비고**에 「실제 분당단가 … (VAT 미포함 기준)」 한 줄로 남겼다. 그 줄이 그 사실이 남는 유일한 자리다.
    - **같이 움직여야 했던 것 둘.** 워크북 「검증」 탭의 산정 크레딧이 `M(공급가) ÷ O × 60` 이었는데, 공급가가 언제나 총액 ÷ 1.1 이 되면서 **모든 행**이 계약 크레딧 ÷ 1.1 로 나와 「크레딧 산정 불일치」로 찍힌다 — `L(계약금액) ÷ O × 60` 으로 고쳤다(살아 있는 워크북에는 `--refresh-derived` 를 돌려야 반영된다). **분납 회차 금액은 다시 깔지 않는다**: 옛 총액으로 깔린 회차가 남아 수금율이 100% 를 넘을 수 있지만, 다시 까는 것은 운영자가 손으로 적은 입금일과 완료 표시를 지우는 일이라 기계가 조용히 할 일이 아니다.
