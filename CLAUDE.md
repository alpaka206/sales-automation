...
- **Contacted 후속 리마인더 — 답이 없으면 3일 · 5일 · 7일** (2026-09-17 운영자 지시, `agents/followup_sequence`,
  ...
  - **몇 차까지 갔는지 티켓에 적고 소통 히스토리에도 남긴다** (2026-09-21 운영자 지시).
    말의 출처는 `followup_sequence.done_label` **한 곳**이고(「1차 리마인더 완료」), 티켓 배너의
    칩과 소통 히스토리 줄과 진행 기록이 같은 문자열을 쓴다 — 자리마다 지으면 한 화면에서
    서로 다르게 읽힌다. 「보냈다」의 기준은 `next_step` 과 같다(`sent` + `sent_at`): 메일
