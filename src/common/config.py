...
class Settings(BaseSettings):
    ...
    # perso.ai@estsoft.com 이 우선이니깐 저런 거 따로 안 해도 돼」). `3114216464` 가 그
    # 주소의 채널 계정 id 이고, 환경변수를 안 걸어도 이 값으로 나갑니다 — 비워 두면 규칙이
    # 아예 안 걸리는데, 그건 **화면 어디에도 안 보입니다.**
    #
    # 그래도 **설정으로 남깁니다.** 이 값은 주소가 아니라 포털이 발급한 id 라, 계정을 다시
    # 연결하면 바뀝니다. 코드에 박으면 그날 발송이 조용히 다른 주소로 나가거나 실패하고,
    # 고치려면 배포를 해야 합니다 — `.env` 한 줄이면 되는 일에.
    HUBSPOT_PREFERRED_EMAIL_CHANNEL_ACCOUNT_ID: str = "3114216464"
    ...
    #
    # **기본값도 그 하나입니다** (2026-09-08). 비워 두면 쓸 수 있는 주소가 전부 뜨고,
    # 거기엔 이 팀이 안 쓰는 `support@perso.ai` 도 섞입니다 — 고르개에 있으면 언젠가
    # 골라지고, 그건 고객이 받는 메일의 보낸사람이 바뀐다는 뜻입니다.
    #
